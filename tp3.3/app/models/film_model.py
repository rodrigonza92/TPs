...
from .exceptions import CustomException, FilmNotFound, InvalidDataError
...
class Film:
    ...
    def create(cls, film):
        ...
        if len(film.title) >= 3 and isinstance(film.laguage_id, int) and isinstance(film.rental_duration,int) and isinstance(data.get('special_features'), list):
    ...
    def exists(self):
        ...
        query = "SELECT IF(EXISTS(SELECT * FROM sakila.film WHERE film_id = %s), 1, 0);"
        params = self.film_id,
